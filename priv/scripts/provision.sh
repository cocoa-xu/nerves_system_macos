...
tart run "$name" --no-graphics >"$session/tart.log" 2>&1 &
vm_pid=$!
cleanup() {
...
sshpass -e ssh "${ssh_options[@]}" "$GUEST_USERNAME@$ip" \
  "env EXPECTED_VERSION=$EXPECTED_VERSION EXPECTED_BUILD=$EXPECTED_BUILD GUEST_USERNAME=$GUEST_USERNAME /bin/bash -s" <"$script"
